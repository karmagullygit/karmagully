...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  String? _validatePinCode(String? value) {
    ...
    if (country == 'india' || country == 'ind' || country == 'in') {
      ...
      final canadaPostalRegex = RegExp(
                    ...
                    if (_selectedAddressId != null)
                      Expanded(
                        child: FutureBuilder<UserAddress?>(
                          future: Future.value(
                            addressProvider.getAddressById(_selectedAddressId!),
                          ),
              ...
              children: [
                Expanded(
                  child: TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText:
                          _countryController.text.toLowerCase().contains(
                            'india',
                          )
                          ? 'PIN Code *'
                          : 'Postal Code *',
  ...
  void _applyCoupon() async {
    ...
    try {
      ...
      if (result == null) {
        ...
        final discount = _appliedCoupon!.calculateDiscount(
  ...
  void _placeOrder(double total) async {
    ...
    try {
      ...
      if (_selectedPaymentMethod != null) {
        // Use Razorpay for all online payments (UPI & Card)
        if (_selectedPaymentMethod == PaymentMethod.upi ||
            _selectedPaymentMethod == PaymentMethod.card) {
          // Use Razorpay for online payments
          final amountPaise = (total * 100).toInt();
          ...
          try {
            ...
            // Open Razorpay checkout with hardened options
            final options = {
              ...
              'prefill': {
                'contact': _phoneController.text.replaceAll(
                  RegExp(r'[^0-9]'),
                  '',
                ),
            ...
            // Payment will be handled by success/error callbacks
            return;
          } catch (err) {
            _showSnackBar('Failed to initiate payment: $err', Colors.red);
            return;
          }
        } else {
          // Process COD payment using simulation
          ...
          paymentInfo = paymentResult.toPaymentInfo(
  ...
  void _handleRazorpaySuccess(PaymentSuccessResponse response) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        // Create order in local provider
        final orderId = await orderProvider.placeOrder(
          ...
          paymentInfo: PaymentInfo(
            method: PaymentMethod.card, // Using card for online payment
                  ...
                  ...presetAddresses.map(
                    (preset) => Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${preset.addressLine1}, ${preset.city}',
                        ),
                        onTap: () async {
                          ...
                          if (userId.isNotEmpty) {
                            final newId = await addressProvider
                                .addPresetAddress(
                                  userId: userId,
                                  presetId: preset.id,
                                  isDefault: false,
                                );
                            final newAddress = addressProvider.getAddressById(
                              newId,
                            );
                            if (newAddress != null) {
                              setState(() {
